...
private:
    ...
    class BindedValueNode : public Node {
    private:
    ...
    class EmbeddedFunctionNode : public Node {
    private:
    ...
    public:
        inline virtual std::shared_ptr<Value> evaluate(const std::vector<std::shared_ptr<Value>> & _arguments) override final {
            if(__Expression->is_return())
                return __Expression->evaluate(_arguments);
            else
                return __nextColon->evaluate(_arguments);
        }
    };
